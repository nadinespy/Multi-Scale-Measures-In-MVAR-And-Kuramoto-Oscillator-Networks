...
if Gnuplot
	...
	writematrix([rmi Fxy],'bivar_ar1_rmi.dat','Delimiter','tab')

	% Run Gnuplot

	system('gnuplot bivar_ar1_rmi.gp');

	% Display PDF plot

	system(sprintf('%s bivar_ar1_rmi.pdf &',pdfviewer));
	...
	plot(rmi,Fxy);
	xscale log
	title(sprintf('Granger causality vs. residuals mutual information (RMI) for a bivariate AR(1) model\n'),'fontsize',14,'interpreter','latex');
	xlabel('RMI','interpreter','latex');
	ylabel('$F(Y \to X)$  ','rotation',0,'interpreter','latex');
	grid on
