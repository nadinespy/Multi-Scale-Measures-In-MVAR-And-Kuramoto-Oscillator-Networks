...
elseif strcmp(measure_in_filename, 'integrated_info')
	measure_in_table = {'IntegratedInfo', 'CausalDensity', ...
		'IntegratedSynergy'};
elseif strcmp(measure_in_filename, 'control')
	measure_in_table = {'TimeDelayedMI', 'AverageCorr'};
...
for g = 1:length(measure_in_table)
	...
	filename_heatmap = {[network '_heatmap_coup_mag_rmi_', ...
		measure_in_table{g}]};
	title_heatmap = {strrep(measure_in_table{g}, '_', ' ')};
...
% big_column = [];
% for f = 1:length(measure_in_table) 
% 	for k = 1:n_samples_noise_corr;
% 		one_sample = big_struct.(measure_in_table{f})
% 		big_column = [big_column; struct2array(big_struct.(measure_in_table{f})(:,k)];
% 	end
% end
% 
% big_column = [];
...
for g = 1:length(measure_in_table);
	...
	for i = 1:length(all_cross_coup_mag);
		...
		for j = 1:length(all_rmi);
			...
			temp_matrix = table2array(results_table(i,j)).(measure_in_table{g});
			%temp_matrix = cell2mat(table2array(results_table(i,j))).(measure_in_table{g});
	...
	save([char(pathout_data_measures), network '_big_array_' measure_in_table{g} '.mat'],'results_table',  '-v7.3');

	figure;
	scatter3(BIG_array(:,1), BIG_array(:,2), BIG_array(:,3), 0.5, ".", 'blue');
	title({strrep(measure_in_table{g}, '_', ' '), ' across RMI and coupling magnitude: for each pair of them,', 'connectivities and noise correlations were randmoly sampled'});
	xlabel('coupling magnitude');
	ylabel('RMI');
	zlabel(strrep(measure_in_table{g}, '_', ' '));
	
	location = [pathout_plots_measures, network '_' measure_in_table{g}, '.png'];
	exportgraphics(gcf, location);
...
for p = 1:length(rmi_index)
	...
	for g = 1:length(measure_in_table);
		...
		for q = 1:(length(all_rmi)*length(all_cross_coup_mag));
			all_measure_values = [all_measure_values; big_struct(q,1).(measure_in_table{g})];
		end
		...
		title({[measure_in_table{g} ' across cross-coupling magnitude for rmi-index = ' num2str(rmi_index(p)) ':'], 'for each pair of them, connectivities & noise correlations', 'were randmoly sampled'});
		xlabel('cross-coupling magnitude');
		ylabel(measure_in_table{g});
		ylim([min_measure max_measure]);
		
		location = [pathout_plots_measures, network '_' 'rmi_index' num2str(rmi_index(p)) '_' measure_in_table{g}, '.png'];
		exportgraphics(gcf, location);
