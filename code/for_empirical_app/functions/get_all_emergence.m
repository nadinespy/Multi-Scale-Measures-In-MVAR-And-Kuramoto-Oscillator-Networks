...
%                                                    neighbours							     
	...
	time_lags		= measure_params.time_lags_for_measure;	
	...
	% put PhiID-measures into separate cell array in [measures] 
	% for get_all_phiidCE_DC_CD()
	phiidMeasures = {};
	
	z = 1;
	while (length(measures) ~= 0) 
		
		if strcmp(measures{z}, 'phiidCE') || strcmp(measures{z}, 'phiidDC') || ...
				strcmp(measures{z}, 'phiidCD')
	...
	% initialise main structure where to store all results
	emergence_struct = struct('model', [], ...
		'time_length', [], ...
		'measure', [], ...
		'method', [], ...
		'time_lag', [], ...
			...
			for z = 1:length(methods);
				...
				for i = 1:length(time_lags);
						...
						for c = 1:length(disc_methods)
							...
							for l = 1:length(bins)
								...
								if iscell(measure) == true
									...
									if any(strcmp(measure, 'phiidCE')) || any(strcmp(measure, 'phiidDC')) || ...
											any(strcmp(measure, 'phiidCD'));
										...
										all_phiidCE_DC_CD = get_all_phiidCE_DC_CD(network, ...
											...
											temp_emergence_struct_phiid(1,1:length(red_funcs)* ...
											length(phiidMeasures)), ...
								...
								elseif strcmp(measure, 'shannonCE') || strcmp(measure, 'shannonDC') || ...
										strcmp(measure, 'shannonCD');
					...
					elseif strcmp(lower(method), 'gaussian')
						...
						fprintf('get all emergence - loop indices: time_length: %d, measure: %d, method: %d, time_lag: %d, ', ...
						j, q, z, i);
