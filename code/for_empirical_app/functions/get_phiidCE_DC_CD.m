...
% get_phiidCE_DC_C() calculates PhiID-based Causal Emergence (CE), 
% Downward Causation (DC), and Causal Decoupling (CD) for any micro 
% variable specified in [micro_variables] and the parameters given.
%
% Takes as inputs the measure (PhiID-CE, PhiID-DC, or PhiID-CD), scalar
% measure parameter values ([time_lag]), method ([method]), PhiID-specific 
% measure parameter [red_func], and returns the PhiID-measure as specified
% in [measure].
%
% Example: output_struct = get_phiidCE_DC_CD(micro_variables, ...
%	     measure, method, time_lag, red_funcs, 'kraskov_param', ...
%	     kraskov_param)
%
% INPUTS - required:	
...
%
% OUTPUT:	
%    all_phiidCE_DC_CD	    -			1 x n struct with with micro
...
%							measures specified in [measure])
	...
	for i = 1:n_micro_variables;
		...
		try
			...
			warning('PhiID-based CE has not worked', method);
			phiidCE = NaN;
			phiidDC = NaN;
			phiidCD = NaN;
		end
