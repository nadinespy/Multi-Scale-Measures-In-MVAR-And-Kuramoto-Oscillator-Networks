%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% mec_km_rand_coup_ratio_phase_lag_rmi_test_script.m
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Before running this script, run 'params_[measure]_rand_phase_lag_rmi.m' 
...
% set seeds (zero for no seeding)
seed_osc_freq = 0; 		% oscillator frequencies random seed
seed_coup_ratio_mean = 0; 	% Shanahan connectivity parameter random seed
seed_noise_corr = 0; 		% oscillator input noise correlation random seed
seed_noise_gen = 0; 		% oscillator input noise generation random seed 
seed_init_phase = 0; 		% oscillator initial phases random seed 
...
% phase lag			
all_phase_lag = pi/2 - linspace(phase_lag_range(1), ...	
...
coup_ratio_mean_dev = coup_ratio_mean_rel_dev * ...		
	coup_ratio_mean;	
	
% total number of oscillators
n_nodes = n_comms * n_nodes_per_comm;				
% oscillator frequencies deviation
osc_freq_dev = osc_freq_rel_dev * osc_freq_mean;		 
% uniform or empirical sampling for local GC values (if true, it's empirical, 
% if false, uniform)
emp_sample = true;
...
% oscillator frequencies Beta distributed on [0,pi] with mean osc_freq_mean 
% and std. dev osc_freq_dev
if osc_freq_dev > 0
	seed_state = rng_seed(seed_osc_freq);
	osc_freq = pi*ms_betarnd(osc_freq_mean/pi, ...			
		osc_freq_dev, n_nodes, 1);					 
...
% sampling frequency
sampling_freq  = 1/dt_sim_points;
% Nyqvist frequency						
nyquist_freq = sampling_freq/2;	
% frequencies in Hz					
freq_hz = nyquist_freq * osc_freq/pi;					


% simulation times
% equilibriation time increments
dt_equil_points = floor(n_equil_points / dt_sim_points);		
assert(dt_equil_points >= 0, 'Equilibriation time too short,' ...
	' or time increment too large!');
% adjusted equilibriation time
n_equil_points = dt_equil_points * dt_sim_points;			

% simulation time increments
sim_time_increments = floor(n_sim_points/dt_sim_points);		
assert(sim_time_increments > 0,'simulation time too short,' ...
	' or time increment too large!');
% adjusted simulation time
n_sim_points = sim_time_increments * dt_sim_points;			

% total simulation time increments
tot_sim_time_increments = dt_equil_points + sim_time_increments;	
...
defvar('gvprog','neato'       ); % GraphViz filter ('dot, 'neato', 'fdp', etc. 
				  % - see man pages; default: 'dot')

% Spectral parameters

defvar('welch_window_width',  []         ); 	% Welch nwindow width
defvar('welch_noverlap', []         ); 	% Welch noverlap
defvar('welch_window_type',  []         ); 	% Welch window type (e.g., @hann, 
						% @flattopwin, @rectwin, etc.; 
						% see Matlab docs)
defvar('welch_nfft',     []         ); 	% Welch nfft = 2*fres

% display connectivity
if disnet
	wgraph2dot(osc_freq/max(osc_freq), weighted_network/ ...
	max(weighted_network(:)), [tempdir 'kuramoto_sh'], true, gvprog, true);
	return
end
...
[autospectra, freq] = cpsdx(detrended_phases, sampling_freq, ...
	welch_window_width, welch_noverlap, welch_window_type, welch_nfft, true);
...
figure(1); clf;
sgtitle(sprintf('Kuramoto system (Shanahan): n_comms = %d, ...
	n_nodes_per_comm = %d, n_inter_comm_coups = %d, \\phase_lag = %g\n', ...
