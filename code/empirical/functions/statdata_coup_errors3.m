function [X, Y] = statdata_coup_errors3(coupling_matrix, npoints, tau, err, corr)

% -----------------------------------------------------------------------
%   FUNCTION: statdata_coup_errors3.m
...
% statdata_coup_errors3() obtains time-series data for a Gaussian MVAR(p) that 
% has correlation [corr], and noise terms with correlation [err]
...
cov_err = diag(standard_dev)*R*diag(standard_dev);	% diag() converts std vector to matrix where the diagonal entries are the stds, 
...
corr_matrix(corr_matrix==0) = corr;

X = mvnrnd(mu,corr_matrix,npoints)';			% simulate time-series with zero mean and given covariance
...
% vectorized version
for t = (1+tau):npoints
	Y(:,t) = coupling_matrix*X(:,t-tau) + E(:,t);
end
