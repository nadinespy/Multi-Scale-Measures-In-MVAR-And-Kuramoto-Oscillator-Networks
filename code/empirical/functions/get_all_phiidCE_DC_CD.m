...
% model parameters.
% 
% Example: output_struct = get_all_phiidCE_DC_CD(network, model_params1, ...
%	     model_params2, time_length, measure, method, time_lag, ...
%	     red_funcs, micro_variable_names, input_struct, ...
%	     pathin_sim_time_series, 'kraskov_param', kraskov_param)
%
% Takes as inputs the measure (PhiID-CE, PhiID-DC, or PhiID-CD), scalar
% measure parameter values ([time_length], [time_lag]), method ([method]),
% model parameter arrays ([model_params1], [model_params2]),
% PhiID-specific measure parameters [red_funcs], and names of the micro 
% variables ([micro_variable_names]).
%	
% INPUTS - required:
...
%                                         described above;
	...
	for e = 1:length(model_params2)
		model_params2_str{e} = num2str(model_params2(e));
	end 
	...
	for u = 1:length(micro_variable_names);
		...
		output_struct(1,z).results(1,1).(micro_variable_names{u}) = ...
			...
			'RowNames', model_params1_str, 'VariableNames', model_params2_str);
		end
	end
			...
			for o = 1:length(model_params2)
				...
				if strcmp(lower(method), 'kraskov') | strcmp(lower(method), 'gaussian');
					...
					for k = 1:length(micro_variable_names)
						micro_variables(1).(micro_variable_names{k}) = struct2array(load([pathin ...
							network '_' ...
					...
					for k = 1:length(micro_variable_names)
						...
						try 
							reconstruction_ica = rica(micro_variables(1).(micro_variable_names{k})', ...
								n_features);
							
							rica_micro = (micro_variables(1).(micro_variable_names{k})' * ...
							...
							warning('dim reduction of micro variables for discrete method in PhiID-based CE was not successful');
				...
				% phiidCE_DC_CD is an array with structs where each struct corresponds to one PhiID-measure 
				% specified in [measure], in the same order
				...
				for h = 1:length(fieldnames_phiidCE_DC_CD)
					...
					for f = 1:length(fieldnames_results)
						...
						output_struct(1,h+y).results(1,1).([fieldnames_results{f}])(n,o) = ...
							{phiidCE_DC_CD(1,1).([fieldnames_phiidCE_DC_CD{h}]).([fieldnames_results{f}])};
					...
					output_struct(1,h+y).model		= network;
