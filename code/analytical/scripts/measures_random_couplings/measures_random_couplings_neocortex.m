function measures_random_couplings_neocortex()
	...
	% pre-allocate memory (populate each cell in cell array with a blank struct)
	results = repmat({zeros(1,1)},length(all_norms), length(all_rmi));

	blank_struct.two_norm = zeros(n_samples_couplings, n_samples_noise_corrs);
	blank_struct.spectral_radius = zeros(n_samples_couplings, n_samples_noise_corrs);
	blank_struct.coupling_matrix = repmat({zeros(1,1)},n_samples_couplings, n_samples_noise_corrs);
	blank_struct.noise_corr = repmat({zeros(1,1)},n_samples_couplings, n_samples_noise_corrs);
	blank_struct.n_nodes = n_nodes;
	blank_struct.m_dim = m_dim;
	blank_struct.multi_info = zeros(n_samples_couplings, n_samples_noise_corrs);
	blank_struct.average_corr = zeros(n_samples_couplings, n_samples_noise_corrs);
	blank_struct.integrated_info = zeros(n_samples_couplings, n_samples_noise_corrs);
	blank_struct.time_delayed_mi = zeros(n_samples_couplings, n_samples_noise_corrs);
	blank_struct.DD_pca = zeros(n_samples_couplings, n_samples_noise_corrs);
	blank_struct.ShannonCE_pca = zeros(n_samples_couplings, n_samples_noise_corrs);
	blank_struct.co_info_pca = zeros(n_samples_couplings, n_samples_noise_corrs);
	blank_struct.DD_grassmanian = zeros(n_samples_couplings, n_samples_noise_corrs);
	blank_struct.ShannonCE_grassmanian = zeros(n_samples_couplings, n_samples_noise_corrs);
	blank_struct.co_info_grassmanian = zeros(n_samples_couplings, n_samples_noise_corrs);

	results = repmat({blank_struct},length(all_norms), length(all_rmi));
	...
	for i = 1:length(all_norms);
		...
		for j = 1:length(all_rmi);
			...
			% pre-allocate memory
			noise_corrs = repmat({zeros(1,1)},n_samples_couplings, n_samples_noise_corrs);
			coupling_matrices = repmat({zeros(1,1)},n_samples_couplings, n_samples_noise_corrs);
			new_norms = zeros(n_samples_couplings, n_samples_noise_corrs);
			spectral_radiuses = zeros(n_samples_couplings, n_samples_noise_corrs);
			
			for x = 1:length(measures);
	                     	if strcmp(measures{x}, 'multi_info')
                                	multi_info = zeros(n_samples_couplings, n_samples_noise_corrs);

                              elseif strcmp(measures{x}, 'DD_CE_CO_INFO')
						for t = 1:length(dim_reduction)
							if strcmp(dim_reduction{t}, 'pca')
								all_DD_pca = zeros(n_samples_couplings, n_samples_noise_corrs);
                                            	all_ShannonCE_pca = zeros(n_samples_couplings, n_samples_noise_corrs);
                                            	all_co_info_pca = zeros(n_samples_couplings, n_samples_noise_corrs);
                                    	elseif strcmp(dim_reduction{t}, 'grassmanian')
								all_DD_grassmanian = zeros(n_samples_couplings, n_samples_noise_corrs);
                                    	    	all_ShannonCE_grassmanian = zeros(n_samples_couplings, n_samples_noise_corrs);
                                            	all_co_info_grassmanian = zeros(n_samples_couplings, n_samples_noise_corrs);
                                    	end
						end

                              elseif strcmp(measures{x}, 'phiid_based_measures_mmi')
						phiidDoubleRed_MMI = zeros(n_samples_couplings, n_samples_noise_corrs);
                                    phiidCE_MMI = zeros(n_samples_couplings, n_samples_noise_corrs);
                                    phiidDC_MMI = zeros(n_samples_couplings, n_samples_noise_corrs);
                                    phiidCD_MMI = zeros(n_samples_couplings, n_samples_noise_corrs);
                                    phiidUC_MMI = zeros(n_samples_couplings, n_samples_noise_corrs);
                                    phiidSyn_MMI = zeros(n_samples_couplings, n_samples_noise_corrs);
                                    phiidTransfer_MMI = zeros(n_samples_couplings, n_samples_noise_corrs);

                              elseif strcmp(measures{x}, 'phiid_based_measures_ccs')
                                    phiidDoubleRed_CCS = zeros(n_samples_couplings, n_samples_noise_corrs);
                                    phiidCE_CCS = zeros(n_samples_couplings, n_samples_noise_corrs);
                                    phiidDC_CCS = zeros(n_samples_couplings, n_samples_noise_corrs);
                                    phiidCD_CCS = zeros(n_samples_couplings, n_samples_noise_corrs);
                                    phiidUC_CCS = zeros(n_samples_couplings, n_samples_noise_corrs);
                                    phiidSyn_CCS = zeros(n_samples_couplings, n_samples_noise_corrs);
						phiidTransfer_CCS = zeros(n_samples_couplings, n_samples_noise_corrs);

					elseif strcmp(measures{x}, 'integrated_info_measures')
						for t = 1:length(calcNames)
							if strcmp(calcNames{t}, 'AverageCorrelation')
								average_corr = zeros(n_samples_couplings, n_samples_noise_corrs);
							elseif strcmp(calcNames{t}, 'IntegratedInformation')
								integrated_info = zeros(n_samples_couplings, n_samples_noise_corrs);
							elseif strcmp(calcNames{t}, 'IntegratedInteraction')
                                        		integrated_interaction = zeros(n_samples_couplings, n_samples_noise_corrs);
							elseif strcmp(calcNames{t}, 'DecoderIntegration')
                                        		decoder_integration = zeros(n_samples_couplings, n_samples_noise_corrs);
							elseif strcmp(calcNames{t}, 'CausalDensity')
                                        		causal_density = zeros(n_samples_couplings, n_samples_noise_corrs);
							elseif strcmp(calcNames{t}, 'TimeDelayedMutualInfo')
                                        		time_delayed_mi = zeros(n_samples_couplings, n_samples_noise_corrs);
							end
						end
					end
			end
						...
						while count == err_count
							...
							try
								...
								elseif strcmp(measures{l}, 'DD_CE_CO_INFO')
									...
									toc_disp = ['running through get_ssi_from_var for one k,p: ', num2str(toc)];
									disp(toc_disp)
									...
									for z = 1:length(dim_reduction)
										...
										toc_disp = ['running through get_ce_and_dd for one k,p: ', num2str(toc)];
										disp(toc_disp)
								...
								elseif strcmp(measures{l}, 'phiid_based_measures_mmi')
									...
									phiidUC_MMI(k,p)					= phiid_atoms_MMI.rts + phiid_atoms_MMI.xts + ...
										phiid_atoms_MMI.yts;
									phiidSyn_MMI(k,p)				= phiidUC_MMI(k,p) + phiidDC_MMI(k,p) + ...
										phiidDoubleSyn_MMI(k,p);
								...
								elseif strcmp(measures{l}, 'phiid_based_measures_ccs')
									...
									phiidUC_CCS(k,p)				= phiid_atoms_CCS.rts + phiid_atoms_CCS.xts + ...
										phiid_atoms_CCS.yts;
									phiidSyn_CCS(k,p)				= phiidUC_CCS(k,p) + phiidDC_CCS(k,p) + ...
										phiidDoubleSyn_CCS(k,p);
			...
			toc_disp = ['going through all k,p: ', num2str(toc)];
			disp(toc_disp)
