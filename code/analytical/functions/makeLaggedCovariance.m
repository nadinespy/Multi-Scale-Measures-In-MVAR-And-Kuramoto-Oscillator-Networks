...
function [ lS ] = makeLaggedCovariance(A, c)
  ...
  N = size(A, 1);
  eps = c*ones(N);                      % 2*2 coupling matrix (i.e., error for one variable is correlated with the error for the other variable)
  eps(1:(N+1):end) = 1;              % changes diagonal entries to 1.0

  S = dlyap(A, eps);                    % solve discrete-time lyapunov equations: ASAT − S + eps = 0; 2*2 covariance matrix (so S is the covariance matrix)
  lS = [S, S*A'; A*S, S];               % 4*4 matrix (the first quadrant should be the covariance matrix)
