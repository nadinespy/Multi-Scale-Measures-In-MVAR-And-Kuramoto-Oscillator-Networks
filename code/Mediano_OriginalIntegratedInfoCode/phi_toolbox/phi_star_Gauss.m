function phi_star = phi_star_Gauss(Cov_X,Cov_XY,Cov_Y,Z,beta_init)
...
%   - The optimization method for beta was changed from steepest descent to
%   a quasi-Newton method by Jun Kitazono
%
...
I_s_d_Const_part = trace(C_D_cond\YX_D/X_D*Cov_XY');
...
[sqrt_beta,minus_I_s,~,~] = minFunc(@I_s_I_s_d,sqrt_beta,Options);
beta = sqrt_beta^2;
I_s = -minus_I_s;
...
phi_star = I - I_s;
    ...
    function [minus_I_s, minus_I_s_d] = I_s_I_s_d(sqrt_beta)
        beta = sqrt_beta.^2;
        ...
        R = - beta*C_D_beta1_inv - beta^2*S_left/Q_inv*S_right;
        ...
        R_d = -C_D_beta1_inv - beta*S_left*2/Q_inv*S_right - beta*S_left*beta*Q_d*S_right;
        
        I_s_d = 1/2*(-trace(Q_inv*Q_d) + trace(Cov_Y*R_d)) + I_s_d_Const_part; % derivative of I_s
        minus_I_s_d = -I_s_d * 2 * sqrt_beta;
    end
