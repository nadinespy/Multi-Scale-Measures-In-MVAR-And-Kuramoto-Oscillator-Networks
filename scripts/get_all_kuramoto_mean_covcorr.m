function get_all_kuramoto_mean_covcorr(network, A_vec, beta_vec, all_npoints, ...
	...
	for q = 1:length(all_npoints);
		...
		save([pathout_data_mean_corr network '_mean_corr_pair_sync_' num2str(npoints) '.mat'], ...
			'mean_corr_pair_sync');
		save([pathout_data_mean_cov network '_mean_cov_pair_sync_' num2str(npoints) '.mat'], ...
			'mean_cov_pair_sync');
