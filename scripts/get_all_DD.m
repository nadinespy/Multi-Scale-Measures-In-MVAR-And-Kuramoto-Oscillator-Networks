function get_all_DD(network, time_series_length, measure_params, measure_params_dd, model_params, ...
	...
	for q = 1:length(time_series_length);
		...
		for z = 1:length(measure_param1);
					...
					for b = 1:length(measure_param2);
						...
						for p = 1:length(measure_param4);
							...
							for o = 1:length(measure_param5);
								...
								fprintf('get_all_DD - loop indices: time_series_length: %d, measure_param1: %d, model_param1: %d, model_param2: %d, measure_param2: %d, measure_param4: %d, measure_param5: %d\n', q, z, i, j, b, p, o);
								...
								DD = get_DD(micro_variables, macro_variables, measure_param2{b}, measure_param1(z), measure_param1_dd, measure_param3);
								
								fieldnames_DD = fieldnames(DD);

								for l = 1:length(fieldnames(DD));
									temp_DD(1).([fieldnames_DD{l,1}])(i,j) = DD.(fieldnames_DD{l,1});
								end
								...
								if strcmp(lower(measure_param2(b)), 'discrete');	
									...
									for l = 1:length(fieldnames_temp_DD);
										new_fieldnames_temp_DD{l} = [fieldnames_temp_DD{l} '_' num2str(measure_param5(o))];
									end
								else 
									% fieldnames consist of measure (practical CE, DC, or CD) + micro variable + macro variable + pract CE method
									for l = 1:length(fieldnames_temp_DD);
										new_fieldnames_temp_DD{l} = [fieldnames_temp_DD{l} '_' measure_param2{b}];
									end
								end
			...
			save([pathout_data_dd network '_all_DD_' time_series_length_str '_' measure_param1_str '.mat'], ...
				'all_DD');
