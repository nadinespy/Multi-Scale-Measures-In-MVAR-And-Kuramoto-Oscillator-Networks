function X = sim_mvar_network(coupling_matrix, npoints, tau, corr_err)
...
% simulate time-series of correlated errors (by multiplying M with the upper triangular 
% matrix L obtained from the Cholesky decomposition of the desired correlation matrix R) 
% but: I get different values for corr, which might be due to the fact that chol might fail, 
...
% but: this can yield different actual correlations for them; the chosen correlation 
% will only be yielded, if sample size is large 
mu = zeros(1, nvar);
...
for i=tau+1:npoints
    for j=1:nvar
        for k=1:nvar
            for m=1:tau
			X(j,i)= (big_coupling_matrix(j,k+(m-1)*nvar)*X(k,i-m)+E(j,i));
            end
        end
    end
end 
...
for t = (1+tau):npoints
	next_value = 0;
	for k = 1:tau
		temp_value = coupling_matrix*X(:,t-k);
...
% here, we consider only one time-point from the past: 
% drawing *at each time-step* error values from a correlated 
