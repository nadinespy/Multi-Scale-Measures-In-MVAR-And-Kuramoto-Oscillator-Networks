...
for i = 1:size(coupling_matrices, 3);
	...
	for j = 1:length(error_vec)
		...
		% PhiID
		phiid_all_err_coup_mmi_(:,i,j) = struct2array(PhiIDFull(X, tau, 'MMI'))';
		phiid_all_err_coup_ccs(:,i,j) = struct2array(PhiIDFull(X, tau, 'ccs'))';
		
		% take the first 2 variables, then add subsequently further ones
		phiid_all_err_coup_mmi_two_var(:,i,j) = struct2array(PhiIDFull(X(1:2,:), tau, 'MMI'))';
		phiid_all_err_coup_ccs_two_var(:,i,j) = struct2array(PhiIDFull(X(1:2,:), tau, 'ccs'))';
