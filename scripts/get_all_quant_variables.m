		...
		for i = 1:length(model_param1);
			...
			for j = 1:length(model_param2)
				...
				for o = 1:length(measure_param5);
					...
					for k = 1:length(micro_variable_names)
						[quantiles_variable, quantilized_variable] = get_quant_variables(micro_variables(1).(micro_variable_names{k}), measure_param5(o));
						quant_micro_variables(1).(['quant' num2str(measure_param5(o)) '_' micro_variable_names{k}]) = quantilized_variable;
					end
					
					% quantilize all macro variables, and store them in another struct 'quant_macro_variables'
					for k = 1:length(macro_variable_names)
						[quantiles_variable, quantilized_variable] = get_quant_variables(macro_variables(1).(macro_variable_names{k}), measure_param5(o));
						quant_macro_variables(1).(['quant' num2str(measure_param5(o)) '_' macro_variable_names{k}]) = quantilized_variable;
					end
					...
					for k = 1:length(quant_micro_variable_names);
						...
						save([pathout_data_sim_time_series network '_' quant_micro_variable_names{k} '_' model_param1_str '_' model_param2_str '_' time_series_length_str '.mat'], ...
					...
					for k = 1:length(quant_macro_variable_names);
						% save single element from struct
						save([pathout_data_sim_time_series network '_' quant_macro_variable_names{k} '_' model_param1_str '_' model_param2_str '_' time_series_length_str '.mat'], ...
