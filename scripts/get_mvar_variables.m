function get_mvar_variables(network, model_sim_params, measure_params, coupling_matrices, ...
		pathout);
...
% use inputParser to declare required & optional variables
	...
	for q = 1:length(time_lengths);
		time_length_str = num2str(time_length(q));
		...
		for i = 1:size(coupling_matrices, 3);
			...
			for j = 1:length(model_param2)
				...
				for t = 1: length(time_lags)
					...
					nodes = sim_mvar_network(time_length(q), model_param2(j), coupling_matrix, time_lags(t));
					
					% MICRO VARIABLES:
					%	- time-series of nodes
					%	- components of phases
					
					% do reconstruction ICA with nodes: first get weights for each variable and each feature (output of rica()
					% will be a matrix of size [size(nodes,2) * number of features], then multiply this matrix with input matrix
					...
					% get sum of nodes
					sum_nodes = zeros(1, time_length(q));
					for k = 1:(size(nodes,1));
						sum_nodes = sum_nodes + nodes(k,:);
					end
					
					% get sum of exponent of nodes
					sum_exp_nodes = zeros(1, time_length(q));
					for k = 1:(size(nodes,1));
						sum_nodes = sum_exp_nodes + exp(nodes(k,:));
					end
					
					% variable names consist of  network name + variable name + value of coupling + value of corr err + number of datapoints + time lag
					save([pathout network '_rica' num2str(n_features) '_nodes_' model_param1_str '_' model_param2_str '_' time_length_str '_' time_lag_str '.mat'], ...
						'-struct', 'S');
					save([pathout network '_sum_nodes_' model_param1_str '_' model_param2_str '_' time_length_str '_' time_lag_str '.mat'], 'sum_nodes');
					save([pathout network '_sum_exp_nodes_' model_param1_str '_' model_param2_str '_' time_length_str '_' time_lag_str '.mat'], 'sum_exp_nodes');
				end
			end
